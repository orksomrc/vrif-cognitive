...
var responseTextField = document.getElementById('QR~' + qidstr);
//create the number	
var number = Math.floor((Math.random() * (99999 - 10001) + 10001))

//set an embedded field that records the randomly generated number
Qualtrics.SurveyEngine.setEmbeddedData('q5bb_number', number)                              
...
var underSpan = document.createElement('span');
	jQuery(underSpan).append("_ _ _ _ _ ").css({"color":"red","fontSize":"24pt","display":"none"}).attr("id","underspan");
	jQuery("#newDiv").append(underSpan);
	
	
var time = 5000; //time in milliseconds after which number block disappears
...
var randNumber = Qualtrics.SurveyEngine.getEmbeddedData('q5bb_number')
...
{
	Qualtrics.SurveyEngine.setEmbeddedData('q5bb_answer',1)
	console.log('result of the question is True')
}
else 
{
	Qualtrics.SurveyEngine.setEmbeddedData('q5bb_answer',0)
