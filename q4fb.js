...
var responseTextField = document.getElementById('QR~' + qidstr);
//create the number	
var number = Math.floor((Math.random() * (9999 - 1001) + 1001))

//set an embedded field that records the randomly generated number
Qualtrics.SurveyEngine.setEmbeddedData('q4fb_number', number)                              
...
var underSpan = document.createElement('span');
	jQuery(underSpan).append("_ _ _ _ ").css({"color":"red","fontSize":"24pt","display":"none"}).attr("id","underspan");
	jQuery("#newDiv").append(underSpan);
	
	
var time = 4000; //time in milliseconds after which number block disappears
...
var randNumber = Qualtrics.SurveyEngine.getEmbeddedData('q4fb_number')                              

/*check if the number variable is the same as the currentResponse variable*/
if(currentResponse == randNumber)
{
	Qualtrics.SurveyEngine.setEmbeddedData('q4fb_answer',1)
	console.log('result of the question is True')
}
else 
{
	Qualtrics.SurveyEngine.setEmbeddedData('q4fb_answer',0)
