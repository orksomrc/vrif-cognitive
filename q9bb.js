...
var responseTextField = document.getElementById('QR~' + qidstr);
//create the number	
var number = Math.floor((Math.random() * (999999999 - 100000001) + 100000001))

//set an embedded field that records the randomly generated number
Qualtrics.SurveyEngine.setEmbeddedData('q9bb_number', number)                              
...
var underSpan = document.createElement('span');
	jQuery(underSpan).append("_ _ _ _ _ _ _ _ _ ").css({"color":"red","fontSize":"24pt","display":"none"}).attr("id","underspan");
	jQuery("#newDiv").append(underSpan);
	
	
var time = 10000; //time in milliseconds after which number block disappears
...
var randNumber = Qualtrics.SurveyEngine.getEmbeddedData('q9bb_number')
var revString = randNumber.toString().split('').reverse().join('')
var revNumber = parseInt(revString,10)


/*check if the number variable is the same as the currentResponse variable*/
if(currentResponse == revNumber)
{
	Qualtrics.SurveyEngine.setEmbeddedData('q9bb_answer',1)
	console.log('result of the question is True')
}
else 
{
	Qualtrics.SurveyEngine.setEmbeddedData('q9bb_answer',0)
